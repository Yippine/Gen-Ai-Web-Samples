<template>
  <div class="max-w-md mx-auto my-8 p-4 rounded-lg shadow-lg relative">
    <button @click="toggleSettings" class="absolute left-4 top-4 text-gray-600 hover:text-gray-800 text-2xl">
    ...
    <div ref="chatContainer" class="h-[calc(100vh-210px)] overflow-y-auto mb-4 bg-white p-2 rounded relative">
    ...
    <button v-show="showScrollButton" @click="scrollToBottom" class="fixed bottom-4 right-4 bg-white rounded-full p-2 shadow-md">
      <i class="fas fa-chevron-down"></i>
    </button>

    <SettingsPanel v-if="isSettingsOpen" @close="toggleSettings" />
  </div>
</template>
...
<script>
  ...
  const MAX_ROWS = 5;
  const LINE_HEIGHT = 20; // 假設每行高度為20px
  ...
  export default {
    ...
    setup() {
      ...
      const handleOutsideClick = (event) => {
        if (isSettingsOpen.value && !event.target.closest(".settings-panel") && !event.target.closest(".fa-bars")) {
